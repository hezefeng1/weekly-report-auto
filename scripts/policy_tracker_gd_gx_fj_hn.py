...
CITY_WEBSITE = {
    # 广东省
    "广州市": "http://rsj.gz.gov.cn",
    "惠州市": "http://rsj.huizhou.gov.cn",
    "江门市": "http://rsj.jiangmen.gov.cn",
    "揭阳市": "http://rsj.jieyang.gov.cn",
    "茂名市": "http://rsj.maoming.gov.cn",
    "梅州市": "http://rsj.meizhou.gov.cn",
    "阳江市": "http://rsj.yangjiang.gov.cn",
    "湛江市": "http://rsj.zhanjiang.gov.cn",
    "中山市": "http://hrss.zs.gov.cn",           # 中山市人社局特殊域名
    "韶关市": "http://rsj.shaoguan.gov.cn",
    "肇庆市": "http://rsj.zhaoqing.gov.cn",
    # 广西壮族自治区
    "桂林市": "http://rsj.guilin.gov.cn",
    "南宁市": "http://rsj.nanning.gov.cn",
    "梧州市": "http://rsj.wuzhou.gov.cn",
    "玉林市": "http://rsj.yulin.gov.cn",
    "来宾市": "http://rsj.laibin.gov.cn",
    "崇左市": "http://rsj.chongzuo.gov.cn",
    "贵港市": "http://rsj.guigang.gov.cn",
    "贺州市": "http://rsj.hezhou.gov.cn",
    "柳州市": "http://rsj.liuzhou.gov.cn",
    "罗城仫佬族自治县": "http://rsj.hechi.gov.cn",   # 属河池市，使用市级官网
    # 福建省
    "漳州市": "http://rsj.zhangzhou.gov.cn",
    # 海南省
    "澄迈县": "http://rsj.chengmai.gov.cn",
    "昌江黎族自治县": "http://rsj.changjiang.gov.cn",
    "海口市": "http://rsj.haikou.gov.cn",
    "临高县": "http://rsj.lingao.gov.cn",
}

def get_website(city):
    if city in CITY_WEBSITE:
        return CITY_WEBSITE[city]
    city_simple = city.replace("市", "").replace("县", "").replace("自治县", "")
    for key in CITY_WEBSITE:
        if key.replace("市", "").replace("县", "").replace("自治县", "") == city_simple:
...
任务：搜索2026年1月1日之后新发布的企业补贴政策，覆盖广东省、广西壮族自治区、福建省、海南省，输出表格格式政策追踪报告。
...
请开始生成报告。"""
    ...
    user_prompt = f"请生成2026年人社补贴政策追踪报告（广东广西福建海南），政策发布日期为2026年1月1日之后，截止当前日期（{today}）仍未过期的政策。严格按照固定格式输出。"
...
def send_rich_text_message(access_token, receive_id, rows, region="广东广西福建海南"):
...
def main():
    ...
    print("\n4. 发送富文本消息...")
    send_rich_text_message(token, RECEIVE_OPEN_ID_POLICY, rows, "广东广西福建海南")
