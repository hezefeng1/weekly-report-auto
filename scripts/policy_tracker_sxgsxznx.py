...
CITY_WEBSITE = {
    # 陕西省
    "宝鸡市": "http://rsj.baoji.gov.cn",
    "汉中市": "http://rsj.hanzhong.gov.cn",
    "渭南市": "http://rsj.weinan.gov.cn",
    "铜川市": "http://rsj.tongchuan.gov.cn",
    "咸阳市": "http://rsj.xianyang.gov.cn",
    "延安市": "http://rsj.yanan.gov.cn",
    # 甘肃省
    "兰州市": "http://rsj.lanzhou.gov.cn",
    "武威市": "http://rsj.wuwei.gov.cn",
    "酒泉市": "http://rsj.jiuquan.gov.cn",
    # 西藏自治区
    "拉萨市": "http://rsj.lasa.gov.cn",
    # 宁夏回族自治区
    "吴忠市": "http://rsj.wuzhong.gov.cn",
}
...
def generate_policy_report():
    """调用 DeepSeek API 生成政策追踪报告（陕西/甘肃/西藏/宁夏）"""
    today = datetime.now().strftime("%Y年%m月%d日")

    system_prompt = """你是人社政策情报分析AI。

任务：搜索2026年1月1日之后新发布的企业补贴政策，覆盖陕西省、甘肃省、西藏自治区、宁夏回族自治区，输出表格格式政策追踪报告。
...
请开始生成报告。"""
    ...
    user_prompt = f"请生成2026年人社补贴政策追踪报告（陕西甘肃西藏宁夏），政策发布日期为2026年1月1日之后，截止当前日期（{today}）仍未过期的政策。严格按照固定格式输出。"
...
def send_rich_text_message(access_token, receive_id, rows, region="陕西甘肃西藏宁夏"):
...
def main():
    ...
    print("\n4. 发送富文本消息...")
    send_rich_text_message(token, RECEIVE_OPEN_ID_POLICY, rows, "陕西甘肃西藏宁夏")
