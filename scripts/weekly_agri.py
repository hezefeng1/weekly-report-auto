...
- **变化描述必须用完整文字**，如"较前周上涨1.3%"、"较前周下跌0.8%"、"较前周持平"，禁止使用 `↑1.3%`、`↓0.8%`、`→` 等符号表达。
...
⚠️ **格式要求：** 每条必须严格使用 `|` 分隔符，格式为 `卡片名：数值 | 变化描述`。
- **变化描述必须用完整文字**，如"较前周上涨1.3%"、"较前周下跌0.8%"、"较前周持平"。
...
⚠️ **强制要求**：必须基于搜索到的信息生成4条具体可操作的建议。严禁输出"暂无行动建议"或类似空内容。如果搜索到的信息不足以生成4条，请基于行业常识补充合理建议。

**格式要求（必须严格遵守）：**
- 使用编号：**01**、**02**、**03**、**04**
- 每条包含：【触发场景】+【行动建议】，分两行书写
