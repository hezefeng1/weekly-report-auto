...
⚠️ **格式要求：** 每条必须严格使用 `|` 分隔符，格式为 `卡片名：数值 | 变化描述`。
- **变化描述必须用完整文字**，如"较前周上涨1.3%"、"较前周下跌0.8%"，禁止使用 `↑1.3%` 符号。
...
**示例：**
- 生猪均价：19.87元/kg | 较前周上涨0.6%
- 猪粮比：8.2:1 | 较前周上升0.1
- 自繁自养利润：612元/头 | 较前周上涨4.6%
- 饲料成本：3,215元/吨 | 较前周下跌0.8%
...
**示例：**
> - 生猪均价19.87元/kg，较前周上涨0.6%，延续窄幅震荡格局
> - 豆粕期货价格回落带动饲料成本下行至3,215元/吨
...
每条格式必须为：`{{序号}} | 【触发场景】{{内容}} | 【行动建议】{{内容}}`
- 序号为 01、02、03、04
- 使用 `|` 分隔，整条在一行，禁止换行
- 必须输出4条，禁止输出"暂无行动建议"

**示例：**
01 | 【触发场景】豆粕期货价格周度下跌1.8% | 【行动建议】建议饲料采购部门适度增加豆粕库存储备
02 | 【触发场景】秋季重大动物疫病防控工作启动 | 【行动建议】建议各养殖场户提前完成秋季疫苗采购与免疫程序制定
