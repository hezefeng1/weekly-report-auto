...
### 二、本周卡片数据（⚠️ 标题必须固定为"## 本周卡片数据"）

⚠️ **格式要求：** 每条必须严格使用 `|` 分隔符，格式为 `- 卡片名：数值 | 变化描述`
- 变化描述用完整文字，如"较前周上涨0.6%"
...
### 七、行动建议（⚠️ 标题必须固定为"## 行动建议"）

⚠️ **格式要求：** 每条必须为 `01 | 【触发场景】{{内容}} | 【行动建议】{{内容}}`
- 整条在一行，使用 `|` 分隔
- 必须输出4条，禁止输出"暂无行动建议"

**示例：**
01 | 【触发场景】豆粕期货价格下跌1.8% | 【行动建议】建议适度增加库存储备
