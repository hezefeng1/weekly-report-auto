...
### 七、行动建议（⚠️ 标题必须固定为"## 行动建议"，内容必须为表格格式）

**格式要求：** 表头固定为 `| 序号 | 触发场景 | 行动建议 |`

⚠️ **强制规则：**
- **"行动建议"列必须是以"建议"开头的具体可执行动作**，如"建议饲料采购部门适度增加豆粕库存储备"
- **禁止在"行动建议"列中只写现象描述**，如"豆粕期货价格周度下跌1.2%" 这种不算行动建议
- **如果某个场景暂时无法给出具体建议，也请基于行业常识补充合理建议，严禁留空或写现象**

**示例：**

| 序号 | 触发场景 | 行动建议 |
|------|---------|---------|
| 01 | 豆粕期货价格周度下跌1.2% | 建议饲料采购部门适度增加豆粕库存储备，锁定低价窗口期 |
| 02 | 生猪价格高位震荡，猪粮比接近调控区间上沿 | 建议养殖端分批有序出栏，避免集中压栏，锁定合理利润 |
...
请开始生成周报。"""
    ...
    user_prompt = f"请生成 {today} 的农牧行业周报，信息时间为最近7天（{last_week} 至 {today}）。严格按照固定格式输出。注意：行动建议表格中的'行动建议'列必须是以'建议'开头的具体可执行动作，禁止只写现象描述。"
