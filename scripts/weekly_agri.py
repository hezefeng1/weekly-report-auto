...
**格式要求：** 表格格式，表头为 `| 序号 | 触发场景 | 行动建议 |`

⚠️ **“行动建议”列必须是具体的、可执行的动作**，如“建议饲料采购部门适度增加豆粕库存储备”、“建议养殖端分批有序出栏，避免压栏”，而不是描述现象。

| 序号 | 触发场景 | 行动建议 |
|------|---------|---------|
| 01 | 豆粕期货价格上涨，饲料成本预期抬升 | 建议饲料采购部门适度增加豆粕库存储备，锁定成本 |
| 02 | 生猪价格高位震荡，猪粮比处于高位 | 建议养殖端分批有序出栏，锁定利润，避免集中压栏 |
