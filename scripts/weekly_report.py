...
- 关键人才争夺：{{最紧缺岗位名称}} | {{供需比或缺口描述}}
- 组织效能：{{头部企业名称}} | {{人效/成本控制变化}}
- 人才结构：{{核心指标}} | {{变化趋势及幅度}}
- 竞品动作：{{企业名称}} | {{当周最重要HR动作}}
...
5. **格式要求**：
   - 每条必须包含 `|` 分隔符，左侧为对象/指标，右侧为数据或动作。
   - 左侧不超过10个字，右侧不超过15个字。
   - 不使用加粗、链接或引用标记。

6. **数据来源**：必须从当周内容中提取，不得凭空编造。若某条数据本周确实没有，可标注 `—`，但要确保至少3条有实际数据。
...
#### 二、行业竞品HR动态
重点分析：牧原股份、温氏股份、海大集团、双胞胎集团、正大集团（中国区）

关注维度：招聘策略对比、人才培养进展、海外人才布局、人效提升、培训体系、薪酬绩效、企业文化、最新简讯

格式要求：
- 必须使用统一维度对比表，横向对比5家企业
- 每条动态后标注信息来源，并提供原文链接
